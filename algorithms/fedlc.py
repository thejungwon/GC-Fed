...
class FedLCClient:
    ...
    def __init__(self, client_id, data_loader, model_parameter, args):
        """
        Initialize a FedLCClient instance.
        ...
        class2data = torch.zeros(n_class, dtype=torch.float)

        for _, labels in data_loader:
            for lbl in labels:
                class2data[lbl] += 1

        # Shape (1, n_class) so we can broadcast easily onto logits of shape (batch_size, n_class)
        self.class2data = class2data.unsqueeze(0).to(args.device)
        self.calibration_temp = self.args.fedlc_calibration_temp
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            for inputs, labels in self.data_loader:
                ...
                # BEGIN FEDLC
                # TODO: need to move to args.py
                # https://github.com/bytedance/FedDecorr/blob/master/approach/fedlogitcal.py

                outputs = outputs - self.calibration_temp * (self.class2data**-0.25)
                # END FEDLC
...
class FedLC:
    ...
    def client_train(self, params):
        ...
        client = FedLCClient(client_id, data_loader, model_parameter, args)
        loss = client.train()
