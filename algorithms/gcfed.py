...
class GCFedClient:
    ...
    def __init__(self, client_id, data_loader, model_parameter, args):
        """
        Initialize a GCFedClient instance.
        ...
        if self.args.gc_target_layer == "":
            self.gc_layer_index = int(
                len(list(self.model.named_parameters())) * self.args.gc_layer_lambda
            )
            self.local_gc_exclusion_layer_name = ""
        else:
            self.local_gc_exclusion_layer_name = self.args.gc_target_layer
            self.gc_layer_index = -1
            ...
            for inputs, labels in self.data_loader:
                ...
                for name, param in list(self.model.named_parameters())[
                    ...
                    if param.grad is not None:
                        if (
                            self.local_gc_exclusion_layer_name != ""
                            and self.local_gc_exclusion_layer_name in name
                        ):
                            continue

                        gradients = param.grad  # original gradient (e.g., fc: MxN)

                        mu_grad = gradients.mean(
                            dim=tuple(range(1, len(list(gradients.size())))),
                            keepdim=True,
                        )

                        param.grad = gradients - mu_grad  # projected gradient
                self.optimizer.step()
...
class GCFed:
    ...
    def aggregate_gradients(self, client_updates):
        ...
        for layer in aggregated_weight.keys():
            ...
            mean_global_delta = global_delta.mean(
                dim=tuple(range(1, len(list(global_delta.size())))),
                keepdim=True,
            )
            # Global Projection
            aggregated_weight[layer] += self.global_lr * (
                global_delta - mean_global_delta
            )
    ...
    def client_train(self, params):
        ...
        client = GCFedClient(client_id, data_loader, model_parameter, args)
        loss = client.train()
